...
| # | 缺陷 | 后果 |
| --- | --- | --- |
| 1 | 指令的 `mounted` 在元素**挂载之后**执行 | 先渲染再删除。按钮上的自动聚焦、埋点上报**已经发生了** |
| 2 | 删掉的元素**不在虚拟 DOM 树上**，`updated` 不会再触发 | 🔴 **权限恢复后按钮回不来**——`VE-5.2` 在指令方案下**无法实现** |
| 3 | 指令值的类型是 `any` | `v-perm="'ticket:ticket:delet'"` **编译通过、静默失效** |
...
```
superadmin 登录 → 注册了 /system/users 等全部路由
     ↓ 登出
cs_staff 登录  → 注册了 /tickets
     ↓ 地址栏输入 /system/users
...
```ts
// ❌ 改了筛选条件，列表**不会**重新请求
queryKey: ['tickets', params.page, params.status]
```

数组字面量在 `<script setup>` 里**只求值一次**。`queryKey` 定死了，
`params` 后续怎么变都不会触发重新请求。

**表现**：换页码、改筛选，表格纹丝不动。没有报错，没有警告，
控制台干干净净。第一次加载还是对的。
