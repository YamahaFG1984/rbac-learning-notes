...
| --- | --- |
| `api/tickets.ts` | 🟢 **逐字相同** |
| `useTableQuery` | 🟡 Vue 更短（~20 行 ← ~30 行，`route.query` 天生响应式） |
| **`queryKey`** | 🔴🔴 **必须 `computed`，写错静默失效**（实测见下） |
| **参数要传 `Ref`** | 🔴 传值的话 composable 从入口就收不到变化 |
| 返回值 | 🟡 `Ref`（script 里 `.value`，模板自动 unref） |
| `columns` | 🔴 必须 `computed` ← React 的 `useMemo` 是**阻止**重算 |
| `type` 而非 `interface` | 🟢 **限制相同**（TS 的规则，与框架无关） |
| 不做二次过滤 | 🟢 **决策相同**，与框架无关 |
| 断言数字 | 🟢 **80 / 50 / 5，实测逐格相同** |
| `replace` 而非 `push` | 🟢 同一个坑，与框架无关 |
| 导出绕过拦截器 | 🟢 **相同的已知瑕疵** |

### 🔴📌 `queryKey` 写错的实测数据

故意把 `queryKey` 写成数组字面量（**真实写法，不加任何 cast**）跑一遍：
...
你会先怀疑后端筛选、怀疑 URL 同步、怀疑分页组件，
最后才想到「请求根本没发出去」。

⚠️ 顺带一个观察：我第一次用 `as unknown as never` 强转来制造这个错误，
结果 **TS 报错了**——差点误以为「类型系统能挡住」。
去掉 cast、用开发者真实会写的形式，才确认它完全不报错。
**制造 bug 的方式如果不真实，得到的结论也不真实。**

### 📌 一个测试写法上的教训

第一版断言用的是「共 N 条」文本。演示数据里 80 条工单**全是 `open`**，
所以筛「已关闭」得到 0 条，而 **antdv 的分页器在 `total === 0` 时整个不渲染**，
那段文本随之消失——断言变得毫无意义（它「通过」了，因为文本确实不含「共 50 条」）。

改用**行数**断言（20 行 → 0 行）才真正证明了「请求重新发过」。

> 又一次：**一条测试如果因为「要找的东西根本不存在」而通过，它什么都没证明。**
> 这已经是本阶段第三次遇到同类问题（`vue-v0.2.0` 的导航顺序、
> `vue-v0.5.0` 的整页重载、这里）。

**本 tag 的结论**：数据层几乎完全复用（`api/tickets.ts` 逐字相同），
数据权限矩阵 80/50/5 与后端 `SCOPE_MATRIX` 和 React 版**逐格相同**。
唯一的实质差异是响应式的接入方式，而它恰好是本阶段**最容易踩且最不报错**的一处。
