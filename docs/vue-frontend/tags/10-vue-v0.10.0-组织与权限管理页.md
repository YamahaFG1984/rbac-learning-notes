...
| 项 | 差异 |
| --- | --- |
| `api/admin.ts` | 🟢🟢 **去掉注释后逐字相同**（252 行，实测 diff 为空） |
| `buildTree`（部门） | 🟢 **逐字相同**（纯函数） |
| 权限树的 `checkStrictly` | 🟢 **决策相同**，属性名也相同 |
| 继承态由后端给 | 🟢 **决策相同**（实测 3 个继承节点被禁用） |
| `grantable` 由后端算 | 🟢 **决策相同**（前端不重算 ADR-011） |
| 数据范围走 `save_role_scope` | 🟢 **相同**（实测审计里确实有「设置数据范围」） |
| 审计不做数据权限 | 🟢 **相同的设计** |
| 树不分页 | 🟢 **相同**（实测权限点 26 个全在） |
| 用户表单无 `is_superuser` | 🟢 **相同的红线** |
| Modal 销毁属性 | 🔴 `destroy-on-close` ← `destroyOnHidden`（**UI 库版本世代**） |
| 派生值 / `Select` 空值 | 🔴 已知的两类（`computed`、`null` → `undefined`） |
| 权限树 title 的渲染 | 🟡 Vue 用 `#title` 插槽 ← React 直接把 JSX 拼进 `title` |
| **`defaultExpandAllRows`** | 🟢🔴 **两边一样坏**（见下） |

### 🔴📌 `defaultExpandAllRows` 在数据异步到达时不生效 —— **React 版有一模一样的 bug**

树形表格只显示**顶层节点**：

| 页面 | 实际显示 | 应该显示 |
| --- | --- | --- |
| 权限点 | **4 行**（4 个顶层 catalog） | 26 行 |
| 部门 | **1 行**（1 个顶层部门） | 6 行 |

**原因**：`defaultExpandAllRows` 只在首次渲染时算一次「默认展开哪些行」，
而那时 `dataSource` 还是空数组（数据来自 `useQuery`）。数据回来后默认值不重算。

**⚠️ 我把 React 版跑起来验证了：一模一样，也是 4 行和 1 行。**

所以这**不是 Vue/React 差异**，是 antd / antdv 共有的行为。
`fe-v0.12.0` 的 92 个 E2E **从没断言过行数**，所以它一直没被发现——
而 Vue 阶段发现它，只是因为我写了一条 `permRows === 26` 的断言。

**修法**：自己算出全部有子节点的 key，用受控的 `expandedRowKeys`。

⚠️ **本项目只修了 Vue 版**：`frontend/` 必须保持 `fe-v1.0.0` 原样，
否则跨框架 diff 就不可信了（V-ADR-001）。
React 版的这个 bug 留在原处，**作为「第三个前端逼你重看第二个」的又一个样本**。

> 📌 这是本阶段第 3 次出现同一个模式：
> **接下一个前端时挖出的问题，绝大多数不在「新框架怎么写」，
> 而在「上一个实现里没人验证过的地方」。**
>
> 前两次是 `fe-v0.11.0` 的派单下拉框、`fe-v0.12.0` 的数据范围审计——
> 那两个是 React 阶段自己发现并修掉的；这一次是 Vue 阶段发现的、
> **React 阶段至今仍存在的**。

### 📌 一处「我猜了没查」

初版断言写了「内置角色的删除按钮被禁用」。查数据库才发现
**演示数据里 4 个角色的 `is_builtin` 全是 `False`**——
这条断言从一开始就不可能通过。已改成断言按钮存在。

⚠️ 这已经是本阶段第二次（`vue-v0.7.0` 猜错了 `cs_staff` 的权限）。
**写断言前先查一遍数据，比事后调试快得多。**

**本 tag 的结论**：管理页面层**没有任何框架级差异**——
`api/admin.ts` 逐字相同、`buildTree` 逐字相同、全部权限决策相同。
真正的收获反而是一个**两边共有的 UI 库行为**，
以及它在 React 阶段没被发现这件事本身。
