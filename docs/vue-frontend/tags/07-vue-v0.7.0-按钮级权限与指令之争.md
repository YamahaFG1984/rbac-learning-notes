...
| **`v-perm` 指令** | 🔴🔴 **Vue 独有的岔路，实测后明确不走**（V-ADR-007） |
| 篡改演示 | 🔴 Vue **不需要专门开口子**（见 04 文档第 15 节） |
| 权限码类型收窄 | 🟢 两边都有；🔴 **但指令方案会把它丢掉，加了类型也救不回来** |
| 「派单」的位置 | 🟢 Vue **直接写对**（行操作列）——React 版 `fe-v0.9.0` 放错了工具栏，`fe-v0.10.0` 才改 |

### 📌 指令三条缺陷的实测数据（`VE-4.5` 的答案）

完整记录见 [`notes/指令方案评估.md`](../notes/指令方案评估.md)。摘要：

**缺陷 1：先渲染再删除。** 用 `cs_staff`（无 delete）登录，两个按钮最终都不可见，
但 `@vue:mounted` 计数器是：
...
指令版的按钮**被创建、被挂载、`mounted` 跑完了**才被移除；
`<Can>` 版（`v-if`）**根本没被创建**。按钮上的 autofocus / 埋点会实打实地执行一遍。

**缺陷 2：删掉之后回不来（致命）。** 运行时把 perms 改成 `['*']`：
...
⚠️ 这条**只有权限在运行时变化时才暴露**，而那是 `vue-v0.11.0` 的事——
**如果先选了指令方案，四个 tag 之后才会发现它是死路。**

**缺陷 3：类型收窄失效，且加了类型也没用。**

规格书的延伸思考 2 问过「能不能通过给指令加类型解决」。
**实测：不能。** 指令声明成 `Directive<HTMLElement, PermCode>` 之后，

```vue
<Button v-perm="'ticket:ticket:delet'" />   <!-- 拼错 -->
```

`vue-tsc` **依然完全通过**，运行时按钮静默消失（用 `cs_manager` 验证：
正确的码 → 1 个存在，拼错的 → 0 个存在）。
`vue-tsc` 不校验模板里指令值的类型。

而 `<Can :perm="...">` 写错**编译不过**。

> `F-ADR-012`：「TypeScript 在编译期挡住权限码 typo，
> **这是 SPA 相比模板版唯一在权限安全性上更强的地方**」。
> 用了指令，这个唯一的优势就原地退回 Django 模板的水平。

**结论：用一层嵌套换三样东西，不划算。**

📌 但结论不是「指令不好」——`v-focus`、`v-resize`、`v-click-outside` 用它都合适，
因为那些场景**不参与条件渲染、不需要类型收窄、也不需要在状态变化后恢复**。
**权限场景恰好把这三条全占了。**
