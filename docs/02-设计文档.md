...
| 状态 | 已定稿 |
| 最后更新 | 2026-07-31 |
| 关联文档 | [00-RBAC理论基础.md](00-RBAC理论基础.md)（前置阅读）、[01-PRD.md](01-PRD.md)、[03-实施计划.md](03-实施计划.md)、[04-横向对比.md](04-横向对比.md) |
...
> 📌 **更正**：本节初稿写着「若依（RuoYi）等主流国产后台用的是取最宽枚举，这是我们刻意偏离之处」。
> 实现完之后我去核对了若依的 `DataScopeAspect` 源码——**那个说法是错的**。
> 若依同样是 OR 并集，同样有 `DATA_SCOPE_ALL` 短路，无匹配角色时拼
> `dept_id = 0`（默认拒绝）。在这一点上我们和它是一致的，不存在偏离。
>
> 「取最宽枚举」是一个真实存在的错误写法，但把它按到某个具体项目头上，
> 是我在没核实的情况下为了让论证更有张力而写的。上面那段推理不需要这个
> 对照也成立。详细的逐项对比见 [04-横向对比.md](04-横向对比.md)。
...
| `path` | CharField(255) | index | `/1/3/7/`，**由 save() 维护，禁止手工赋值**。用于子树查询；⚠️ 它是字符串，`ORDER BY path` **不等于**先序遍历（`/2/6/10/` < `/2/6/7/`），树的显示顺序在内存里按 `order_num` 排 |
