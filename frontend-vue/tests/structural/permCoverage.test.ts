...
// 🟡 本文件从 React 版移植，改了两处：扫描后缀（.ts|.vue ← .tsx?），
//    以及**先去掉注释再扫**——见下面 stripComments 的说明。
...
/**
 * ⚠️📌 **必须先去掉注释再扫**（`vue-v0.13.0` 实测发现）。
 *
 *    不去掉的话，注释里举例用的 `PERM.X` 会被当成真实引用，
 *    然后因为 `PERM['X']` 是 undefined 而误报：
 *
 *      offenders: ["undefined（用于 src/auth/usePermission.ts）"]
 *
 *    ⚠️ **React 版的扫描器有同样的弱点**，只是它的源码里恰好
 *       没有出现「注释里写 PERM.XXX」的情况，所以一直没被触发。
 *       这是本阶段第 4 次「接下一个前端时发现上一个实现里没人验证过的地方」。
 *
 *    → 这也是为什么本文件**不是**「只改后缀」的逐字移植。
 */
